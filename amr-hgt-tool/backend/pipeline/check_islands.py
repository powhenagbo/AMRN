...
gene-overlap step entirely. This answers "does island detection find
ANYTHING at all" separately from "do any of those islands happen to
contain an AMR gene", which are two different questions that looking at
a gene's 'score' field alone conflates.

Usage:
    cd ~/AMRN/amr-hgt-tool/backend
    python check_islands.py /path/to/genome.fasta
"""
